...
* **`configService.get<_____>('...')`:** El método `get` de NestJS acepta un *Genérico* en TypeScript (los diamantes `< >`). Esto le sirve a TypeScript para saber qué tipo de dato primitivo debe esperar de la variable de entorno y darnos autocompletado sin errores.
...
```
*(Asegúrate de importar este `YoutubeModule` en el array de imports de tu `app.module.ts` principal).*
