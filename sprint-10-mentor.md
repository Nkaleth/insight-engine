...
**El Por Qué (Visión Senior):** Evitamos el error de novato de hacer un simple `axios.get` en un bucle for (lo cual causaría un baneo de IP casi inmediato por _Rate Limiting_). Encapsulamos la lógica en un _Provider_ para aislar el control de peticiones de la lógica central de la app.
...
> _"Implementé un servicio de extracción aislado utilizando `@nestjs/axios`. En lugar de llamar a las APIs directamente desde los controladores, encapsulé la lógica en un Provider que maneja sus propias inyecciones de configuración (`ConfigService`) asegurando el principio de Fail Fast mediante `getOrThrow`. Además, utilicé RxJS (`firstValueFrom`) para adaptar el paradigma reactivo de NestJS al flujo asíncrono estándar, lo que me permite controlar los identificadores de sesión (User-Agent) de forma centralizada y prevenir baneos por rate-limiting."_

## Notas de Arquitectura Avanzada: Rastreabilidad de Origen (Cerrando el Ciclo)

Para asegurar que en el futuro podamos **responder automáticamente** o navegar manualmente a los comentarios/posts extraídos de Reddit o YouTube, la arquitectura de Base de Datos (Prisma) debe estar preparada para guardar dos atributos clave por cada _Insight_:

1. **`sourceUrl`**: La URL web exacta y navegable. En Reddit se arma con el `permalink` (ej. `reddit.com/r/...`). En YouTube se arma usando el `videoId` y el parámetro de comentario enlazado `lc` (ej. `youtube.com/watch?v={videoId}&lc={commentId}`).
2. **`sourceId`**: El identificador interno único de la plataforma (ej. `t1_k9zxy1` en Reddit, o `Ugz_xxx...` en YouTube). Este campo es **vital** si en el futuro decides programar un bot (usando OAuth), ya que la API necesitará este ID exacto para saber a qué nodo de la red responder.

_El principio de un ingeniero Staff es siempre guardar los datos en bruto que permitan conectividad futura._
